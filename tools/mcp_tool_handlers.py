...
def _get_mcp_delay_config() -> tuple[int, list[str]]:
    ...
    now = time.monotonic()
    cached_at, _stored_ttl, cached_delay, cached_patterns = _mcp_delay_cache
    if now - cached_at < _MCP_DELAY_CACHE_TTL:
    ...
    _mcp_delay_cache = (now, _MCP_DELAY_CACHE_TTL, delay_ms, patterns)
