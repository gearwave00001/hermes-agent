...
def _preamble(evt: dict, title: str, intro: str, completed_at: float, *, with_goal: bool) -> "list[str]":
    ...
    _provider = evt.get("provider")
    model_line = f"Role: {evt.get('role') or 'leaf'}   Model: {evt.get('model') or '?'}"
    if _provider and str(_provider) not in ("?", ""):
        model_line += f"   Provider: {_provider}"
...
def _format_batch_delegation(evt: dict, deleg_id: str, completed_at: float) -> str:
    ...
    # End-of-run routing summary: each subagent with its server IP/provider,
    # so the parent can see which backend ran each task.
    if results:
        lines.append("")
        ...
        for r in sorted(results, key=lambda x: x.get("task_index", 0)):
            ...
            topic = str(r_goal)[:43] + ".." if len(str(r_goal)) > 45 else str(r_goal)
            r_provider = str(r.get("provider") or "?")
            r_model = str(r.get("model") or "?")
            model_short = r_model.split("/")[-1] if "/" in r_model else r_model
    ...
    for r in sorted(results, key=lambda x: x.get("task_index", 0)):
        ...
        header = (f"--- {icon} TASK {idx + 1}/{n}" + (f": {r_goal}" if r_goal else "") + f"  (status={r_status}"
                  ...
                  + (f", provider={r_provider}" if r_provider and r_provider != "?" else "")
