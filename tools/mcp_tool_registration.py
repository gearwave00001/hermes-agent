...
def _tool_candidates(name: str, tools: Iterable[Any], should_register: Callable[[str], bool],
    ...
    for t in tools:
        ...
        # Wrap fetchWebContent with the Camofox browser fallback: JS-rendered /
        # no-content pages fall back to a real browser navigation + snapshot.
        if t.name == "fetchWebContent":
            handler = _make_fetchWebContent_browser_fallback(handler, name, t.name)
