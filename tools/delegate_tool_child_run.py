...
def _build_result_entry(
    ...
    entry: Dict[str, Any] = {
        ...
        "model": _str_or_none(getattr(child, "model", None)),
        # Routed provider (subagent priority routing) or the child's effective
        # provider — surfaced in completion notifications ("Provider: ...").
        "provider": _str_or_none(
            getattr(child, "_assigned_provider", None) or getattr(child, "provider", None)
        ),
...
class _ChildRun:
    ...
    def cleanup(self, *, heartbeat: _Heartbeat, child_pool: Any, leased_cred_id: Any, close_deferred: bool) -> None:
        ...
        # Release the subagent-router provider slot (priority routing): the
        # child was tagged with its routed provider at build time; releasing
        # here frees capacity for queued overflow tasks exactly once.
        ...
        if assigned_provider:
            try:
                ...
                _router_release(str(assigned_provider))
            except Exception as exc:
