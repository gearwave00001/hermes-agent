...
from tools.delegate_tool_dispatch import _Batch, _announce_batch, _capture_origin, _execute_and_aggregate, _run_batch
...
def _build_children(
    ...
    live_deleg_id: Optional[str], live_writers: list, background: bool = False,
    task_images: Optional[List[Optional[List[str]]]] = None,
) -> tuple[List[tuple], List[dict], Optional[str]]:
    """Build every child on the main thread (construction is not thread-safe).

    Returns ``(children, queued_tasks, error)``: ``([], [], error)`` on an
    explicit-pin preflight failure; ``queued_tasks`` holds overflow entries
    (tasks whose provider slot was unavailable at build time) when subagent
    priority routing is enabled — see tools/subagent_router.py. Each entry is
    ``{"task": t, "sync_event": Event|None}``; sync callers wait on the events
    so the tool result still reflects those tasks."""
    from tools.delegation_live_log import wrap_progress_callback
    from tools.delegation_output_schema import append_output_contract
    # Subagent priority routing (opt-in): acquire a provider per child with
    # check-out counters / health checks / overflow queue. When disabled or
    # unavailable, every child shares the resolved creds (existing behavior).
    ...
    try:
        from tools.subagent_router import (
            ...
            _is_enabled as _router_is_enabled,
        )
        _routing_enabled = _router_is_enabled()
    except Exception as exc:
    ...
    for i, t in enumerate(task_list):
        # Per-task credential resolution: routing picks a provider (explicit
        # per-task override > goal rules > priority order); otherwise inherit.
        child_creds = creds
        assigned_provider: Optional[str] = None
        if _routing_enabled:
            child_creds = _acquire_provider(
                provider_override=t.get("provider"), goal=t.get("goal", ""),
            )
            if child_creds is None:
                # No provider slot right now — park in the overflow queue.
                sync_event = threading.Event() if not background else None
                enqueued = _enqueue_task(
                    task=t, session_key=_session_key, sync_event=sync_event,
                    parent_agent=parent_agent, delegation_cfg=routing_cfg,
                )
                if enqueued:
                    queued_tasks.append({"task": t, "sync_event": sync_event, "index": i})
                    logger.debug(
                        "Task %d/%d enqueued for later dispatch (no provider slot): %s",
                        i + 1, len(task_list), t.get("goal", "")[:80],
                    )
                    continue
                # Queue full — fall back to the shared default credentials.
                logger.warning(
                    "Subagent queue full for task %d/%d; falling back to default provider: %s",
                    i + 1, len(task_list), t.get("goal", "")[:80],
                )
                child_creds = creds
            else:
                assigned_provider = child_creds.get("provider")
        overrides = dict(base_overrides)
        if child_creds is not creds:
            overrides.update({
                "override_provider": child_creds.get("provider"),
                "override_base_url": child_creds.get("base_url"),
        ...
        try:
            child = _build_child_preserving_parent_tools(
                ...
                model=(child_creds or creds)["model"], max_iterations=max_iterations, task_count=len(task_list),
                parent_agent=parent_agent, role=_normalize_role(t.get("role") or top_role), **overrides,
            )
        except ValueError as exc:
            # Release the acquired slot before aborting the whole batch.
            if assigned_provider:
                try:
                    ...
                    logger.debug("Failed to release provider '%s' after build error: %s", assigned_provider, rel_exc)
            return [], [], str(exc)
        # Tag the child with its routed provider so run.cleanup() releases the
        # slot exactly once when the child finishes (or fails).
        if assigned_provider:
    ...
    return children, queued_tasks, None
...
def delegate_task(
    ...
    children, queued_tasks, err = _build_children(
        task_list, task_schemas, creds, top_role=top_role, max_iterations=default_max_iter, parent_agent=parent_agent,
        routing_cfg=routing_cfg, live_deleg_id=live_deleg_id, live_writers=live_writers, background=background,
        task_images=task_images,
    )
    if err:
        return tool_error(err)
    # Overflow queue (subagent priority routing): every task parked for later
    # dispatch — no child was built for any of them. Sync callers wait on the
    # dispatcher's completion events; async callers return a "queued" handle
    # and each task re-enters the conversation when it completes.
    if children and not queued_tasks:
        batch = _Batch(
    ...
    if not children:
        if not background:
            logger.debug(
                "All %d tasks queued — waiting for subagent router dispatcher", len(queued_tasks),
            )
            for qt in queued_tasks:
                if qt["sync_event"] is not None:
                    qt["sync_event"].wait(timeout=600)  # 10 min timeout
            return json.dumps({
                "status": "completed",
                "results": [
                    {
                        "task_index": qt.get("index", idx),
                        "status": "queued_completed",
                        "summary": "Task was queued and completed via subagent router",
        ...
        return json.dumps({
            "status": "dispatched",
            "results": [
                {"task_index": qt.get("index", idx), "status": "queued", "goal": qt["task"].get("goal", "")}
                for idx, qt in enumerate(queued_tasks)
            ],
            "note": (
                f"All {len(queued_tasks)} tasks were queued due to provider capacity limits. "
                "Results will re-enter the conversation as each task completes."
            ),
        }, ensure_ascii=False)
    # Mixed case: some children built, some parked in the overflow queue. Run
    # the built children normally; sync callers additionally wait for the
    # queued ones and append their completion markers to the result.
    batch = _Batch(
        task_list, children, parent_agent, creds, context, top_role, max_children,
        live_deleg_id, live_writers, live_paths, *origin, overall_start,
    )
    ...
    if queued_tasks:
        for qt in queued_tasks:
            if qt["sync_event"] is not None:
                qt["sync_event"].wait(timeout=600)
        combined.setdefault("results", []).extend(
            {
                "task_index": qt.get("index", i),
                "status": "queued_completed",
                "summary": "Task was queued and completed via subagent router",
                "goal": qt["task"].get("goal", ""),
            }
            for i, qt in enumerate(queued_tasks)
        )
        combined["note"] = (
            f"{len(queued_tasks)} task(s) exceeded provider capacity at dispatch time and "
            "were completed later by the subagent router dispatcher."
        )
