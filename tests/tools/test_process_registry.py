...
def test_drain_notifications_logs_skipped_and_dropped():
    ...
    surfaced in the CLI. Upstream's drain loop counts skips/drops and emits ONE
    summary line ("drained N events (X formatted, Y skipped, Z dropped)") plus a
    DEBUG line per formatted event — assert on those.
    """
    import logging
    from tools.process_registry import process_registry
    ...
    try:
        ...
        # The consumed completion must be counted as skipped in the drain summary
        assert any("1 formatted, 1 skipped" in msg for msg in log_messages), (
            f"No skip count in drain summary: {log_messages}"
        )
