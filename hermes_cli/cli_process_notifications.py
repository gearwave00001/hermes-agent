...
class CLIProcessNotificationsMixin:
    def _owns_process_notification(self, event: dict) -> bool:
        ...
        if str(resolved_key) == current_key:
            return True
        # Ancestor ownership: child sessions inherit ownership of delegations
        # dispatched from their parents (e.g. /branch forks). Walk the
        # parent_session_id chain upward from the current session; if the
        # event's session appears anywhere on that chain, this session owns it.
        try:
            session_db = getattr(self, "_session_db", None)
